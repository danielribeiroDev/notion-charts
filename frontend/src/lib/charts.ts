...
export function mapApiChartToConfig(api: ChartApiModel): ChartConfig {
  ...
  const metrics: ChartMetric[] = metricsRaw.map((m: any, idx: number) => ({
    ...
    name: typeof metricNames[idx] === 'string' ? metricNames[idx] : `Metric ${idx + 1}`,
...
export async function fetchEmbedData(chartId: string): Promise<EmbedData> {
  const BASE_URL = import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:3000';
  const response = await fetch(`${BASE_URL}/embed/${chartId}`, {
    method: 'GET',
  ...
  if (!response.ok) {
    ...
    throw new Error(message || 'Chart not found');
  }
...
export function buildConfigJson(params: {
  ...
  const metricsPayload: MetricConfigPayload[] = params.metrics.map((m) => {
    metricNames.push(m.name || 'Metric');
