...
function unwrapFirst<T>(enveloped: Envelope<T>): T {
    ...
    if (payload === undefined || payload === null) {
        throw new Error('Unexpected server response (empty body).');
    }
    return payload as T;
}
...
async function parseError(response: Response) {
    ...
    if (response.status === 401) {
        return 'Token invalid or expired. Check the Internal Integration Token.';
    }
    return response.statusText || 'Unknown error while contacting the Notion API.';
}
...
export async function exchangeNotionCode(code: string, workspaceId: string) {
    ...
    if (!accessToken) {
        throw new Error('You must be authenticated to connect to Notion.');
    }
...
function extractPageTitle(page: unknown) {
    if (!page || typeof page !== 'object') return 'Untitled page';
    ...
    if (typeof typedPage.url === 'string') {
        ...
        return slug.replace(/-/g, ' ') || 'Untitled page';
    }
...
export async function fetchDatabaseSum(
    ...
    const dateFilter = (() => {
        ...
        if (!dateProperty) {
            throw new Error('Select a date column to apply the time filter.');
        }
