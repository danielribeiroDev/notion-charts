...
export function validateMetrics(metrics: ChartMetric[]) {
    if (metrics.length === 0 || metrics.some((m) => !m.valueColumn)) {
        return { ok: false, message: 'Adicione pelo menos uma métrica com coluna numérica.' };
    }

    const missingDate = metrics.some((m) => (m.timeRange && m.timeRange !== 'none') && !m.dateProperty);
    if (missingDate) {
        return { ok: false, message: 'Selecione a coluna de data para toda métrica com filtro de tempo.' };
    }
