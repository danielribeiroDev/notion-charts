...
export default function NotionCallback() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            if (!workspaceId) {
                setStatus('error');
                setMessage('WorkspaceId não informado.');
                return;
            }
            try {
                setStatus('loading');
                setMessage('Trocando código por credenciais...');
                const response = await exchangeNotionCode(code, workspaceId);

                if (!response?.ok) {
                    throw new Error('A troca de código não retornou sucesso.');
                }
                ...
                setStatus('success');
                setMessage('Conexão realizada! Redirecionando...');
                navigate('/dashboard', { replace: true });
            } catch (err) {
                const errorMessage = err instanceof Error ? err.message : 'Falha ao conectar com o Notion.';
                setStatus('error');
        ...
        <div className="flex min-h-screen items-center justify-center bg-background px-4">
            <Card className="w-full max-w-md border-border">
                <CardHeader className="flex items-center gap-3">
                    {icon}
                    <div>
                        <CardTitle>Conectando ao Notion</CardTitle>
