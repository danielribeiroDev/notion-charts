            ...
            <CardHeader className="flex flex-row items-start justify-between space-y-0">
                <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                        ...
                        <CardDescription>Card de Estatísticas</CardDescription>
                    </div>
                </div>
                <DropdownMenu>
                        ...
                        return (
                            <div key={metric.id} className="rounded-lg border border-dashed border-border bg-secondary/40 p-4">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm font-semibold">{metric.name || `Métrica ${idx + 1}`}</p>
                                ...
                                {hasData && (
                                    <div className="mt-2 space-y-1">
                                        ...
                                        <p className="text-2xl font-semibold">
                                            {metricAgg.total?.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 }) ?? '0'}
                                        </p>
                                        <p className="text-xs text-muted-foreground">{metricAgg.count ?? 0} registros</p>
                                    </div>
                                )}

                                {!metricAgg && (
                                    <p className="mt-2 text-sm text-muted-foreground">Configure coluna e filtro para ver o total.</p>
                                )}
                            </div>
                        );
                    })}
